...
public class StringUtils {
  ...
  public static String convertMetaMaskAddress(String input) {
    if (!StringUtils.isEmpty(input) && input.length() >= 8) {
      return input.substring(0, 4) + "..." + input.substring(input.length() - 3, input.length());
    } else {
      return input;
    }
  }
